<x-app-layout>
    <x-slot name="header">
        {{ __('Halaman Utama Table Supplier') }}
    </x-slot>
            ...
            <div>
                <div class="p-4 dark:bg-dark-800">
                    <button class="px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-blue-600 border border-transparent rounded-lg active:bg-blue-600 hover:bg-blue-700 focus:outline-none focus:shadow-outline-blue">
                        <a href="{{ route('supplier.create') }}" type="button"><i class="fas fa-plus"></i> Tambah Data</a>
                    </button>
                </div>
                        ...
                        <table class="whitespace-no-wrap w-full">
                            <thead>
                                <tr class="text-xs font-semibold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 dark:text-gray-400 dark:bg-gray-900">
                                    ...
                                    <th class="px-4 py-3">Kode Kode Supplier</th>
                                    <th class="px-4 py-3">Nama Supplier</th>
                                    <th class="px-4 py-3">Nomor HP/WA</th>
                            ...
                            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                                ...
                                <tr class="text-gray-400 text-center">
                                    ...
                                    <td class="px-4 py-3">{{ $data->namaSupplier }}</td>
                                    <td class="px-4 py-3">{{ $data->nomorHp }}</td>
                                    <td class="px-4 py-3">{{ $data->alamat }}</td>
                                    <td class="px-4 py-3">
                                        <div class="flex justify-center space-x-1">
                                            <a href="{{ route('supplier.edit', $data->id) }}" class="flex items-center px-2 py-2 text-sm font-medium leading-5 text-gray-800 bg-white-300 rounded-lg dark:text-gray-500 hover:dark:text-yellow-300 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
